...
local function set_cell(grid, x, y, ch, hl)
  ...
  if iy >= 1 and iy <= #grid and ix >= 1 and ix <= #grid[iy] then
...
local function draw_ring(grid, cr, cc, radius, rows, max_cols)
  ...
  local max_r = dist(rows, max_cols)
...
local config = {
  ...
  init = function(grid)
    ...
    state.center_r = clamp(base_r + jr, 2, math.max(2, state.rows - 1))
    state.center_c = clamp(base_c + jc, 2, math.max(2, state.max_cols - 1))
  ...
  update = function(grid)
    ...
    if state.phase == "implode" then
      ...
      for _, p in ipairs(state.particles) do
        ...
        if frac < 1 then all_done = false end
    ...
    elseif state.phase == "explode" then
      ...
      local max_r = dist(state.rows, state.max_cols)
