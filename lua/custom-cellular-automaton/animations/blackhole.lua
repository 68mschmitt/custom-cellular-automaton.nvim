	...
	for _, p in ipairs(parts) do
		if p.state == "stuck" then
			...
			if p.r0 <= #grid and p.c0 <= #grid[p.r0] then
				U.plot(grid, p.c0, p.r0, p.ch, p.hl)
			end
		end
	end
end
		...
		for c = cmin, cmax do
			...
			if c <= #grid[r] then
				...
				if d <= radius + bands - 1 then
					...
					U.plot(grid, c, r, HOLE_CHARS[band], HEAT_PALETTE[palette_index(coldness)])
				end
			end
		end
	end
...
local function start_bigbang(rows, max_cols)
	...
	galaxy_radius = math.max(0.5, math.min((max_cols - 1) / 2, (rows - 1) / ASPECT / 2) * GALAXY_RADIUS_FRAC)

	stars = {}
	for _ = 1, math.max(24, math.min(total, 1600)) do
		...
		table.insert(stars, {
			...
			coldness = u,
			radius = math.min(GALAXY_MIN_RADIUS, galaxy_radius * 0.2) + u * galaxy_radius * 0.8,
			base_angle = (arm - 1) * (2 * math.pi / GALAXY_ARMS) + GALAXY_TWIST * u + (math.random() * 2 - 1) * scatter,
...
local cfg = {
	...
	init = function(grid)
		...
		for _, p in ipairs(parts) do
			...
			p.detach_at = clamp(exp_sample(mean) + jitter, 0.1, DETACH_MEAN + DETACH_JITTER)
			p._r0_math = nil
		end
	end,
}
...
cfg.update = function(grid)
	...
	for key, val in pairs(placed) do
		...
		if rr <= #grid and cc <= #grid[rr] then
			U.plot(grid, cc, rr, val.ch, val.color)
		end
	end
